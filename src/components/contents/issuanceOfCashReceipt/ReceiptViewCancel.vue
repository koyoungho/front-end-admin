...
<script lang="ts">
    ...
    export default class ReceiptViewCancel extends Vue {
        ...
        listItem: any =  // 그리드 서치 페이징 옵션 처리 데이터 매우중요 이룰을 어기면 화면깨짐이 발생합니다
            {
                ...
                search: [
                    ...
                    {type: 'select' , title :'발급용도',id: 'issuePurpose', name:'issuePurpose' , value: '' ,  api : '' , option : [{ name : '소득공제' , value: '0' },{name : '지출증빙' , value: '1' }]},
                    {type: 'select' , title :'거래',id: 'dealType', name:'dealType' , value: '' ,  api : '' , option : [{ name : '승인' , value: '0' },{name : '취소' , value: '1' }]},
                    {type: 'select' , title :'발급경로',id: 'onlineYn', name:'onlineYn' , value: '' ,  api : '' , option : [{ name : '온라인' , value: ' Y' },{name : '오프라인' , value: 'N' }]},
        ...
        // 뷰페이지 클릭이벤트 받아서 여는곳
        listViewEvent(row){
            this.$router.push({ name:'receiptViewCancelDetl', params: { current : row.searchOption , objectKey : row.row } }) // 라우터 주소를 넣어줘야 히스토리모드 인식
        }
