<template>
    <!-- container -->
    <section id="container">
        <!-- content  -->
        <div class="content">
            <h2 class="blind">시스템 관리</h2>

            <!-- cont_mobile -->
            <div class="cont_mobile page_system0603">
                <!-- title area -->
                <div class="title_area">
                    <div class="fleft">
                        <h3>시스템 모니터링</h3>
                    </div>
                    <div class="fright">
                        <!-- system_state box -->
                        <div class="system_state_box">
                            <span class="text"><i class="state st_color04"></i>아주 좋음</span>
                            <span class="text"><i class="state st_color03"></i>좋음</span>
                            <span class="text"><i class="state st_color02"></i>보통</span>
                            <span class="text"><i class="state st_color05"></i>나쁨</span>
                        </div>
                    </div>
                </div>

                <!-- tab box -->
                <div class="tab_box">
                    <ul class="tab01 col04">
                        <li><a href="#">승인 현황</a></li>
                        <li><a href="#">배치 파일 처리 현황</a></li>
                        <li class="on"><a href="#">시스템 자원</a></li>
                        <li><a href="#">서비스 상태</a></li>
                    </ul>
                </div>

                <h4 class="blind">시스템 자원</h4>

                <!-- system box -->
                <div class="system_box w100">
                    <!-- system info -->
                    <div class="system_info first">
                        <span class="title">FEP #1</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color04">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color03">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->

                    <!-- system info -->
                    <div class="system_info">
                        <span class="title">FEP #2</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color05">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->

                    <!-- system info -->
                    <div class="system_info">
                        <span class="title">승인서버</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color04">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->
                </div>

                <!-- 2번쨰 줄 -->
                <!-- system box -->
                <div class="system_box w100">
                    <!-- system info -->
                    <div class="system_info">
                        <span class="title">정산서버</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color04">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->

                    <!-- system info -->
                    <div class="system_info">
                        <span class="title">WEB #1</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color04">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->

                    <!-- system info -->
                    <div class="system_info">
                        <span class="title">WAS #1</span>
                        <ul class="system_list">
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color04">
                                    <div class="system_col01">
                                        <h5 class="system">CPU 사용률</h5>
                                        <span class="text_type04">CPU 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">메모리 사용률</h5>
                                        <span class="text_type04">메모리 최대</span>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                        <span class="num">90%</span>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <!-- system row -->
                                <div class="system_row st_color02">
                                    <div class="system_col01">
                                        <h5 class="system">저장공간 사용률</h5>
                                    </div>
                                    <div class="system_col02">
                                        <span class="total">60%</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!-- //system info -->
                </div>

            </div>
            <!-- //cont_mobile -->

        </div>
        <!-- //content -->
    </section>
    <!-- //container -->
</template>

<script lang="ts">

    import {Component, Vue} from "vue-property-decorator";

    @Component({
        components: {
            name: "ResourceSystem"

        }
    })
    export default class ResourceSystem extends Vue {

    }
</script>

<style scoped>

</style>