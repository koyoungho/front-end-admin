            ...
            <div class="search_reg_box">
                ...
                <ul class="search_list col03">
                    ...
                    <li>
                        ...
                        <input type="text" class="input form_w100" value="신일약국" title="가맹점명" disabled="disabled" v-model="gajumShopNm">
                    </li>
                    <li>
                    ...
                    <li>
                        <label for="">사업자 번호</label>
                        <input type="text" class="input form_w100" value="0000000000" title="사업자번호" disabled="disabled" v-model="jijumSaupId">
                    </li>
                    <li>
                        <label for="">지점명</label>
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <div class="form_col approval">
                                ...
                                <input type="text" class="input form_app01" placeholder="시작점(0000000)" title="승인대역 시작점" v-model="apro.aproBandFrom" v-bind:disabled="apro.inputDisGbn" maxlength="7">
                                <span class="period_form">-</span>
                                <input type="text" class="input form_app01" placeholder="끝점(1000000)" title="승인대역 끝점" v-model="apro.aproBandTo" v-bind:disabled="apro.inputDisGbn" maxlength="7">
                            </div>
                            <div class="form_col approval">
                                ...
                                <input type="text" class="input form_app02" placeholder="건수(1000건 단위)" value="" title="승인대역 시작점" v-model="apro.aproCnt" v-bind:disabled="apro.inputDisGbn" maxlength="7">
                            </div>
                        </td>
                    </tr>
                    </tbody>
                    ...
                    <div class="tbl_view_box">
                        <table class="tbl_view01">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="input form_w100" title="최종접속일시" disabled="disabled" v-model="adm.lastConnDate">
                                </td>
                            </tr>
                            <tr>
...
<script lang="ts">
    ...
    @Component({
        components: {
            BranchDetl, AddressBox, CompanyCodePop
        },
    })
    export default class BranchDetl extends Vue {
        ...
        gajumId: any = ''; //가맹점 번호
        gajumSaupId: any = ''; //가맹점 사업자번호
        gajumShopNm: any = ''; //가맹점명
        jijumId: any = ''; //지점 번호
        jijumSaupId: any = ''; //지점 사업자번호
        jijumShopNm: any = ''; //지점명
        saupNo: any = ''; //사업자 번호
        soluNm: any = ''; //가맹점명
        ...
        franchiseView(){
            ...
            if(!this.objectKey){
                ...
                this.jijumId = this.objectKey.jijumId; //지점ID

                CommonBoardService.getListDatas('jijum/'+this.gajumId+'/'+this.jijumId, null, null).then((response) => {
                    ...
                    if(result != null){
                        ...
                        if(sessionStorage.role == '0001'||sessionStorage.role == '0002'||sessionStorage.role == '0003') {
                            let jijumStsID = document.getElementById('jijumStatusID')
                            if(jijumStsID!=null){ jijumStsID.removeAttribute('disabled'); }
                        }
                        //승인대역 정보
...
임시 주석 처리 20190113 */
        ...
        updateInfo() {
            ...
            saupData['storSts'] = this.jijumStatus; //지점상태
...
임시 주석 처리 20190113 */
            ...
            reqData['reqGbn'] = 'branch'; //요청구분(필수값)
            reqData['nextPage'] = 'branchList'; //본인인증 후 완료 URL(필수값)

            //console.log('최종 등록 정보 확인');
            reqData['accounts'] = addData3; //관리자 정보 셋팅

            reqData['jijumId'] = this.saupId; //지점번호
            reqData['reqGbn'] = 'branch'; //요청구분(필수값)
            reqData['nextPage'] = 'branchList'; //본인인증 후 완료 URL(필수값)
            ...
            if(confirm('지점 정보를 수정하시겠습니까?')){

                CommonBoardService.updateListData('jijum/'+this.gajumId+'/'+this.jijumId, null, reqData).then((response) => {
                        let result: any = response.data;
